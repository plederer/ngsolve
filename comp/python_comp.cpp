...
void NGS_DLL_HEADER ExportNgcomp(py::module &m)
{
  ...
  // the global variable glh should be replaced by LocalHeapProvider
  static LocalHeap glh(10*1000*1000, "python-comp lh", true);
...
integrator : ngsolve.fem.LFI
  ...
  */
  ...
  py::class_<BDDCPreconditioner<double>, shared_ptr<BDDCPreconditioner<double>>, BASE_BDDCPreconditioner>(m,"BDDCPreconditioner_double");
  py::class_<BDDCPreconditioner<Complex>, shared_ptr<BDDCPreconditioner<Complex>>, BASE_BDDCPreconditioner>(m,"BDDCPreconditioner_complex");  
  ...
  prec_multigrid
    .def(py::init([prec_multigrid](shared_ptr<BilinearForm> bfa, const string& name, optional<shared_ptr<BaseMatrix>> lo_precond, py::kwargs kwargs)
                  {
